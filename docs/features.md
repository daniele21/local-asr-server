...
| Job trascrizione e policy workload | Esegue trascrizioni/analisi come job monitorabili e cancellabili, serializza i workload pesanti e dà priorità alla registrazione attiva. | `JobStore`, `TranscriptionJobManager`, `AnalysisJobManager`, `HeavyWorkloadArbiter` e `runtime/resource_policy.py`. `RecordingStore.active_recording()` è l'owner canonico dello stato capture; `ResourcePolicy` è solo admission policy. L'arbiter controlla la policy sia alla submit sia prima dell'esecuzione, quindi un job accodato non può partire se nel frattempo inizia una registrazione; non esiste un secondo scheduler. I job visuali on-demand riusano lo stesso manager/arbiter. Nel normale Meeting `useMeetingJobEvents` segue `/v1/jobs/{id}/events`; `jobEvents.ts` usa `EventSource` come normal path e un GET snapshot solo dopo stream failure. Il wizard tecnico/import resta separato. | Job ed eventi in SQLite; `job_events` conserva al massimo 512 eventi per job con sequence monotona. Con store persistito `TranscriptionJobManager` non duplica gli eventi in una queue in-memory; la queue process-local resta solo per manager senza store. Una rejection di policy termina il job con failure di `resource_admission`/reason `capture_active`. Il cancel resta cooperativo `cancelling -> cancelled`; job non terminali diventano `interrupted` al restart. | `test_workload_arbiter.py`, `test_resource_policy.py`, `test_job_store.py`, `test_frontend_job_events.py`, `test_recording_api.py`, `test_visual_on_demand.py` e suite job/analysis/transcription. |
