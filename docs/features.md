...
| Job trascrizione, analisi e preparazione Meeting | Esegue trascrizioni/analisi come job monitorabili e cancellabili e offre nel Meeting una sola azione durevole `Prepara note` che riusa risultati validi, rende il transcript leggibile appena pronto e consente recovery esplicito senza duplicare lavoro. | `JobStore`, `TranscriptionJobManager`, `AnalysisJobManager`, `MeetingPreparationService`, `HeavyWorkloadArbiter` e `runtime/resource_policy.py`. Il parent `meeting_preparation` persiste la composizione e i link agli stessi child job esistenti; non esegue inferenza e non introduce queue/worker pesanti. `RecordingStore.active_recording()` resta l'owner canonico dello stato capture e `ResourcePolicy` solo dell'admission. Nel Meeting `useMeetingJobEvents` segue unicamente l'SSE del parent quando la preparazione è attiva, ricarica lo stato canonico al milestone transcript-ready e a terminale, mentre `jobEvents.ts` usa GET snapshot solo dopo stream failure. Gli endpoint expert di trascrizione/analisi restano compatibili. | Job, eventi e relazioni parent→child sono in SQLite. La dedupe durevole usa identità di recording/source, opzioni ASR e template/analisi; un transcript valido evita nuovo ASR, mentre output derivati da un transcript diverso non sono presentati come correnti. `job_events` resta bounded a 512 eventi/job. Il cancel è cooperativo e impedisce stage futuri; job incompleti diventano `interrupted` al restart e ripartono soltanto tramite resume esplicito dal primo stage mancante. | `test_workload_arbiter.py`, `test_resource_policy.py`, `test_job_store.py`, `test_meeting_preparation.py`, `test_frontend_job_events.py`, `test_recording_api.py`, suite job/analysis/transcription, frontend lint/typecheck e browser FULL_MEDIA `meeting-preparation-recovery`. |
