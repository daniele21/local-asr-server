...
| --- | --- | --- | --- | --- |
| Avvio server locale | Espone ClosedRoom come servizio locale per registrare, trascrivere e analizzare audio con accesso protetto same-origin, mantenendo separato il backend dev dall'app macOS. In modalità LLM locale gestita il sidecar resta cold all'avvio e parte solo alla prima fase AI che lo richiede. | CLI `local-asr serve`, risoluzione porta in `cli.py` (`1236` normale, `1237` con `--reload` salvo `--port` esplicito), `create_app()`, endpoint pubblici `/health` e `/v1/session`, static serving, middleware auth locale; `RuntimeServiceManager.ensure_llm_ready()` resta l'owner dell'avvio on-demand del sidecar. | Token sessione generato in app state o `LOCAL_ASR_API_TOKEN`; CORS solo da `LOCAL_ASR_ALLOWED_ORIGINS`; cache in `.cache/` dev o `~/Library/Caches/ClosedRoom/` bundle. | `UV_CACHE_DIR=.cache/uv uv run local-asr serve --reload`, `curl http://127.0.0.1:1237/health`, `UV_CACHE_DIR=.cache/uv uv run python -m unittest discover -s test -p 'test_cli.py' -v`, `test_server_resource_policy_contract.py` e bootstrap cookie via `/v1/session` prima delle API protette. |
...
| Registrazione audio locale | Salva audio progressivamente senza avviare automaticamente AI pesante e permette di recuperare sessioni interrotte. Nel normale New Meeting titolo/progetto sono opzionali e ClosedRoom prova mic + audio computer senza chiedere backend, device, diarizzazione o visual intelligence; sorgenti/device compaiono solo come recovery quando l'automatico non è disponibile. | `NewRecordingPage`, `useRecorder`, `RecordingStore`, endpoint recording/chunk/stop/recovery. Il meter visibile limita il redraw a circa 12,5 Hz, gli aggiornamenti React dei livelli a 4 Hz, timer a 1 Hz e overlay a 2 Hz; il documento hidden salta il lavoro del meter senza cambiare capture/finalizzazione. `/v1/recordings/active`, overlay e `/health` derivano lo stato dalla registrazione persistita, non da flag globali FastAPI. | `<recordings_dir>/<data>/<uuid>/metadata.json` con ledger chunk `sequence`/SHA-256/dimensione; file `.part` finche non finalizzati; stati `interrupted` e `recoverable`; metadati anche in `CatalogStore`. | `test_recordings.py`, `test_recording_api.py`, `test_frontend_new_meeting_simplicity.py`, `test_frontend_recording_efficiency.py` e frontend lint/typecheck. |
| Cattura nativa macOS | Registra microfono e audio computer senza configurazione BlackHole quando macOS 13+, permessi macOS e helper nativo sono disponibili, rendendo evidente il fallback solo quando serve. | Helper Swift `native_capture_helper` con AVFoundation e ScreenCaptureKit, `NativeCaptureManager`, endpoint `/v1/capture/capabilities`, `/v1/capture/permissions`, `/v1/capture/request-permissions`, `/v1/capture/ensure-permissions`, `/v1/capture/diagnostics` e `/v1/recordings/{id}/capture/*` (incluso l'event-stream di volume in tempo reale); il New Meeting usa il backend nativo automaticamente quando pronto e mostra selezione sorgente/device solo nel recovery del fallback. Il preflight richiede i permessi necessari prima della cattura e timer/overlay partono solo sull'evento `ready`. | Tracce WAV `mic.wav`, `system.wav` (16 kHz mono downsampled) e `recording.wav` (mixed via ffmpeg post-stop); `RecordingStore.finalize()` preserva i WAV scritti direttamente dal helper quando esistono placeholder `.part` vuoti; capability/permission/diagnostics separate per Screen Recording/Microfono; `capture_backend`, `capture_status`, `timeline.json`, `quality_report.json`; packaging helper invariato. | `test_native_capture.py`, `test_recordings.py`, `test_paths.py`; frontend checks quando cambia New Meeting; `./build.sh --no-dmg` quando cambia helper/bundle. |
...
| Job trascrizione e policy workload | Esegue trascrizioni/analisi come job monitorabili e cancellabili, serializza i workload pesanti e dà priorità alla registrazione attiva. | `JobStore`, `TranscriptionJobManager`, `AnalysisJobManager`, `HeavyWorkloadArbiter` e `runtime/resource_policy.py`. `RecordingStore.active_recording()` è l'owner canonico dello stato capture; `ResourcePolicy` è solo admission policy. L'arbiter controlla la policy sia alla submit sia prima dell'esecuzione, quindi un job accodato non può partire se nel frattempo inizia una registrazione; non esiste un secondo scheduler. Frontend Transcription continua temporaneamente a usare polling fino a PRS-8. | Job/eventi in SQLite; queue process-local bounded; una rejection di policy termina il job con failure di `resource_admission`/reason `capture_active`. Il cancel resta cooperativo `cancelling -> cancelled`; job non terminali diventano `interrupted` al restart. | `test_workload_arbiter.py`, `test_resource_policy.py`, `test_job_store.py`, `test_recording_api.py` e suite job/analysis/transcription. |
...
| Visual intelligence post-meeting | Backend e artefatti visuali restano disponibili, ma il normale New Meeting non espone più visual capture: la capability è classificata on-demand e la nuova UX esplicita viene completata in PRS-6. Le registrazioni che dispongono già di frame possono ancora elaborarli post-meeting con Qwen3-VL; la pipeline deduplica e preserva evidenze/retry senza face recognition. | `visual_intelligence/`, `PostMeetingVisualService`, API visual-frame/v2 visual intelligence/debug e consumer Meeting/Results. `HeavyWorkloadArbiter` possiede admission/serializzazione; `ModelRuntimeLeaseManager` resta phase marker; Qwen residency viene acquisita solo quando necessaria e rilasciata in `finally`. I vecchi/tecnici workflow che dispongono di frame restano compatibili; New Meeting non seleziona più finestra o modello visuale. | JPEG e manifest esistenti restano in `.visual-staging/`; artefatti promossi in `visual-runs/<generation-id>/` e `current_visual_generation.json`. Cleanup TTL rimuove checkpoint/generazioni incomplete, non i frame. Settings visuali restano compatibili in Advanced durante la migrazione verso on-demand. | `test_visual_intelligence_service.py`, `test_frontend_diagnostics.py`, `test_recording_api.py`, frontend checks; PRS-6 aggiungerà acceptance del nuovo ingresso on-demand. |
...
per un eventuale run futuro. Durante l'elaborazione la UI espone inoltre
`Interrompi elaborazione`: per le registrazioni richiede il cancel del job
persistente e la pipeline si arresta al successivo confine sicuro, inclusa la
fine dell'inferenza visuale corrente; per gli upload interrompe direttamente
lo stream HTTP.
