...
class HeavyWorkloadArbiter:
    ...
    def __init__(self, *, max_concurrent: int = DEFAULT_MAX_CONCURRENT, queue_capacity: int = DEFAULT_QUEUE_CAPACITY) -> None:
        if max_concurrent < 1:
    ...
    @classmethod
    def from_env(cls) -> "HeavyWorkloadArbiter":
        return cls(
    ...
    def submit(
        ...
        *,
        ...
        item = _WorkItem(task_id=task_id, workload_type=workload_type, run=run, on_cancel=on_cancel)
        with self._lock:
            if self._closed:
